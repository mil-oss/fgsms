...
public class Ifconfig extends SigarCommandBase implements Closable {
    ...
    public NetworkRate NetworkSendRate(String interfacename) throws SigarException, InterruptedException {
        NetInterfaceStat ifstat =
                this.sigar.getNetInterfaceStat(interfacename);
        long rx = ifstat.getRxBytes();
        long tx = ifstat.getTxBytes();
        Thread.sleep(1000);
        ifstat =
                this.sigar.getNetInterfaceStat(interfacename);
        rx = ifstat.getRxBytes() - rx;
        tx = ifstat.getTxBytes() - tx;
        NetworkRate r = new NetworkRate();
        r.rx = rx;
        r.tx = tx;
        return r;
