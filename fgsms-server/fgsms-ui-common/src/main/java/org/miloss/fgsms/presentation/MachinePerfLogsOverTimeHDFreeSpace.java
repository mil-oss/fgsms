...
public class MachinePerfLogsOverTimeHDFreeSpace implements DatasetProducer, Serializable {
    ...
    public Object produceDataset(Map params) throws DatasetProduceException {
        ...
        try {
            ...
            if (res != null && res.getMachinePerformanceData() != null && !res.getMachinePerformanceData().isEmpty()) {
                for (int i = 0; i < res.getMachinePerformanceData().size(); i++) {
                    ...
                    for (int k = 0; k < res.getMachinePerformanceData().get(i).getDriveInformation().size(); k++) {
                        ...
                        t2.ResponseTime = res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getFreespace();
                ...
                for (int x = 0; x < dataRX.stuff.size(); x++) {
                    TimeSeries rx = new TimeSeries("Free Space on " + dataRX.stuff.get(x).item, org.jfree.data.time.Millisecond.class);
            ...
            LogHelper.getLog().log(Level.ERROR, "MachinePerfLogsOverTimeHDFreeSpace produce chart data " + ex.getLocalizedMessage());
        }
        return col;
    }
