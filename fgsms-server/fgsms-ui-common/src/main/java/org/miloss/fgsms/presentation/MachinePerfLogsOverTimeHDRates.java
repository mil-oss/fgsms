...
public class MachinePerfLogsOverTimeHDRates implements DatasetProducer, Serializable {
    ...
    TimeSeriesCollection col = new TimeSeriesCollection();
        try {
            ...
            if (res != null && res.getMachinePerformanceData() != null && !res.getMachinePerformanceData().isEmpty()) {
                for (int i = 0; i < res.getMachinePerformanceData().size(); i++) {
                    ...
                    for (int k = 0; k < res.getMachinePerformanceData().get(i).getDriveInformation().size(); k++) {
                        ...
                        t2.ResponseTime = res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getKilobytespersecondDiskRead();
                         t2.cal = res.getMachinePerformanceData().get(i).getTimestamp();
                        if (dataRX.Contains(res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getPartition())) {
                            dataRX.get(res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getPartition()).add(t2);
                        } else {
                            RateStruct t3 = new RateStruct();
                            t3.item = res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getPartition();
                        ...
                        t2.ResponseTime = res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getKilobytespersecondDiskWrite();
                         t2.cal = res.getMachinePerformanceData().get(i).getTimestamp();
                        if (dataTX.Contains(res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getPartition())) {
                            dataTX.get(res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getPartition()).add(t2);
                        } else {
                            RateStruct t3 = new RateStruct();
                            t3.item = res.getMachinePerformanceData().get(i).getDriveInformation().get(k).getPartition();
                            t3.data = new ArrayList<TransactionLogTimeStampStruct>();
                ...
                for (int x = 0; x < dataRX.stuff.size(); x++) {
                    TimeSeries rx = new TimeSeries("Read on " + dataRX.stuff.get(x).item, org.jfree.data.time.Millisecond.class);
                ...
                for (int x = 0; x < dataTX.stuff.size(); x++) {
                    TimeSeries rx = new TimeSeries("Writes on " + dataTX.stuff.get(x).item, org.jfree.data.time.Millisecond.class);
            ...
            LogHelper.getLog().log(Level.ERROR, "MachinePerfLogsOverTimeHDRates produce chart data " + ex.getLocalizedMessage());
        }
        return col;
    }
    ...
    public String getProducerId() {
        return "fgsms.MachinePerfLogsOverTimeHDRates";
    }
}
