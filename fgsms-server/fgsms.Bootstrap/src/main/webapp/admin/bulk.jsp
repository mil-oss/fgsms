...
<%
    ...
    if (request.getMethod().equalsIgnoreCase("post")) {
        try {
            DeleteServicePolicyRequestMsg req1 = new DeleteServicePolicyRequestMsg();
            req1.setClassification(c);
            req1.setDeletePerformanceData(true);
            String[] items = request.getParameterValues("remove_item");
            if (items != null) {
                for (int i = 0; i < items.length; i++) {
                    req1.setURL(items[i]);
                    pcs.deleteServicePolicy(req1);
                }
...
<div class="well">
    <h1>Bulk Delete</h1>
    <p>This will wipe out all selected services, their policies, SLA subscriptions, record status and performance data. Use with caution!.</p>
</div>
<div class="row-fluid">
    <div class="span12">
