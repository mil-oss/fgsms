...
public class ResponseMessageSizeLessThan implements SLARuleInterface {
    ...
    public boolean CheckTransactionalRule(AddDataRequestMsg req, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg) {
        ...
        if (req.getResponseSize() < val) {
            nullableFaultMsg.set("The response message size, " + req.getResponseSize() + ", is less that the value " + val + ", " + nullableFaultMsg.get());
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "Response message size is less than value";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "If a transaction's response message size is less than the specified value, then this rule will trigger.<Br>"
