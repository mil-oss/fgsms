...
public class FaultsOverTimeGreaterThan implements SLARuleInterface {
    ...
    public boolean CheckNonTransactionalRule(ServicePolicy pol, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg, boolean pooled) {
        ...
        long faultrate = NonTransactionalSLAProcessor.GrabFaultRate(duration, pol.getURL());
        if (faultrate > rate) {
            nullableFaultMsg.set("Faults Over Time measured value of " + faultrate + " is greater than " + rate + ", " + nullableFaultMsg.get());
            return true;
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "Faults over time is greater than (value)";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "This rule will trigger when the number of faulty transactions over time is greater than the specified value for the specified duration."
