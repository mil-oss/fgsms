...
public class ResponseMessageSizeGreaterThan implements SLARuleInterface {
    ...
    public boolean CheckTransactionalRule(AddDataRequestMsg req, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg) {
        ...
        if (req.getResponseSize() > val) {
            nullableFaultMsg.set("The response message size, " + req.getResponseSize() + ", is greater that the value " + val + ", "+ nullableFaultMsg.get());
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "Response message size is greater than value";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "If a transaction's response message size is greater than the specified value, then this rule will trigger.<Br>"
                + "Applies to transactional service policies only.<br><br>"
                + "Required parameters:"
                + "<ul>"
                + "<li>value - a positive integer to compare against  (bytes)</li>"
                + "</ul>";
    }
    ...
    public String GetHtmlFormattedDisplay(List<NameValuePair> params) {
        ...
        return Utility.encodeHTML(GetDisplayName() + " " + item + " bytes");
    }
