...
public class HighCPUUsageOverTime  implements SLARuleInterface {
    ...
    public boolean CheckNonTransactionalRule(ServicePolicy pol, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg, boolean pooled) {
        ...
        long faultrate = NonTransactionalSLAProcessor.GetCPUUsageOverTime(pol.getURL(),duration, pooled);
        if (faultrate > rate) {
            nullableFaultMsg.set("CPU Usage measured value of " + faultrate + " is greater than the threshold of " + rate + ", " + nullableFaultMsg.get());
            return true;
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "High CPU Usage over time";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "This rule will trigger when the CPU utilization is greater than the specified value for the specified duration."
                ...
                + "This rule applies to both Machine and Process level policies.<br><br>"
                + "Required parameters:"
                + "<ul>"
                + "<li>value - must be a positive integer. This is the threshold in percentage, 0-100</li>"
        ...
        for (int i = 0; i < params.size(); i++) {
            if (params.get(i).getName().equals("value")) {
                ...
                try {
                    ...
                    if (x <= 0 || x > 100) {
                        outError.set("The parameter 'value' must be greater than zero and 100 or less. " + outError.get());
                    }
                } catch (Exception ex) {
