...
public class ResponseContentContainsIgnoreCase implements SLARuleInterface {
    ...
    public boolean CheckTransactionalRule(AddDataRequestMsg req, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg) {
        ...
        if (req.getXmlResponse() != null
                && req.getXmlResponse().toLowerCase().contains(item.toLowerCase())) {
            nullableFaultMsg.set("The response content contained the flag " + item + ". " + nullableFaultMsg.get());
            return true;
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "Response Content Contains, case insensitive";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "If the message content of the response of a transactional service contains (value), case insensitive, then this rule will trigger.<Br>"
    ...
    public boolean ValidateConfiguration(List<NameValuePair> params, AtomicReference<String> outError, ServicePolicy policy) {
        ...
        if ((policy instanceof TransactionalWebServicePolicy)) {
            TransactionalWebServicePolicy twp = (TransactionalWebServicePolicy) policy;
            if (!twp.isRecordResponseMessage() && !twp.isRecordFaultsOnly()) {
                outError.set("This rule only works when the response is recorded. " + outError.get());
            }
        }
