...
public class ConsumerContainsIgnoreCase implements SLARuleInterface {
    ...
    public boolean CheckTransactionalRule(AddDataRequestMsg req, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg) {
        ...
        for (int i=0; i <req.getIdentity().size(); i++){
            if (req.getIdentity().get(i).toLowerCase().contains(item.toLowerCase())) {
            nullableFaultMsg.set("Consumer Identity Contains Ignoring Case " + item + ", " + nullableFaultMsg.get());
        }
        }
    ...
    public String GetDisplayName() {
        return "Consumer Identity Contains (value), case insensitive";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "If the identity of the consumer of a transactional service contains (value), case insensitive, then this rule will trigger. Consumer identity "
                + "can either be IP addresses, usernames, or the result of an XPath expression.<Br>"
