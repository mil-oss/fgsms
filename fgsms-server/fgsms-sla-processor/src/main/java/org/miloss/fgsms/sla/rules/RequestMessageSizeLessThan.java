...
public class RequestMessageSizeLessThan implements SLARuleInterface {
    ...
    public boolean CheckTransactionalRule(AddDataRequestMsg req, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg) {
        ...
        if (req.getRequestSize() < val) {
            nullableFaultMsg.set("The request message size, " + req.getRequestSize() + ", is less that the value " + val + ", "+ nullableFaultMsg.get());
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "Request message size is less than value";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "If a transaction's request message size is less than the specified value, then this rule will trigger.<Br>"
