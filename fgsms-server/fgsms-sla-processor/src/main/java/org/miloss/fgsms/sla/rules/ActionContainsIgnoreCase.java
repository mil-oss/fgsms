...
public class ActionContainsIgnoreCase implements SLARuleInterface {
    ...
    public boolean CheckTransactionalRule(AddDataRequestMsg req, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg) {
        ...
        if (req.getAction().toLowerCase().contains(item.toLowerCase())) {
            nullableFaultMsg.set("Action Contains Ignore Case " + item + ", " + nullableFaultMsg.get());
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "Action or Method contains (value), case insensitive";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "If the SOAP Action or Method contains (value), case insensitive, then this rule will trigger.<Br>"
    ...
    public boolean ValidateConfiguration(List<NameValuePair> params, AtomicReference<String> outError, ServicePolicy policy) {
        ...
        if (params == null || params.isEmpty()) {
            outError.set("The parameter 'value' is required for rule " + this.GetDisplayName() +". " + outError.get());
        }
        ...
        for (int i = 0; i < params.size(); i++) {
            if (params.get(i).getName().equals("value")) {
                ...
                if (Utility.stringIsNullOrEmpty(params.get(i).getValue())) {
                    outError.set("A value must be specified for the parameter 'value' for rule " + this.GetDisplayName() +". " + outError.get());
                }
            }
        }
        if (!foundLogger) {
            outError.set("The parameter 'value' is required " + this.GetDisplayName() +". "  + outError.get());
        }
