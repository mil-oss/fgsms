...
public class RequestContentContainsIgnoreCase  implements SLARuleInterface {
    ...
    public boolean CheckTransactionalRule(AddDataRequestMsg req, List<NameValuePair> params, AtomicReference<String> nullableFaultMsg) {
        ...
        if (req.getXmlRequest() != null
                && req.getXmlRequest().toLowerCase().contains(item.toLowerCase())) {
            nullableFaultMsg.set("The request content contained the flag " + item + ". " + nullableFaultMsg.get());
            return true;
        }
        return false;
    }
    ...
    public String GetDisplayName() {
        return "Request Content Contains, case insensitive";
    }

    @Override
    public String GetHtmlFormattedHelp() {
        return "If the message content of the request of a transactional service contains (value), case insensitive, then this rule will trigger.<Br>"
    ...
    public boolean ValidateConfiguration(List<NameValuePair> params, AtomicReference<String> outError, ServicePolicy policy) {
        ...
        if ((policy instanceof TransactionalWebServicePolicy)) {
            ...
            if (!twp.isRecordRequestMessage() && !twp.isRecordFaultsOnly()) {
                outError.set("This rule only works when the request is recorded. " + outError.get());
            }
        }
