...
public class AverageResponseTimeByServiceByMethod extends BaseWebServiceReport {
    ...
    public void generateReport(OutputStreamWriter data, List<String> urls, String path, List<String> files, TimeRange range, String currentuser, SecurityWrapper classification, WebServiceContext ctx) throws IOException {
        ...
        try {
            ...
            //add description
            data.append("<table class=\"table table-hover\"><tr><th>URL</th><th>Action</th><th>Average Response Time (ms)</th></tr>");
            int actioncount = 0;
                ...
                for (int k = 0; k < actions.size(); k++) {
                    ...
                    try {
                        cmd = con.prepareStatement(
                                "select AVG(responsetimems)  from RawData where URI=? and "
                    ...
                    data.append(Utility.encodeHTML(actions.get(k))).append("</td><td>").append(count + "").append(" ms</td></tr>");
            ...
            data.append("</table>");
            try {
                ChartUtilities.saveChartAsPNG(new File(path + getFilePathDelimitor() + "image_" + this.getClass().getSimpleName() + ".png"), chart, 1500, pixelHeightCalc(actioncount));
