 ...
 * @author AO
 */
public class SMXJMXScheduler implements StatefulJob {

    private static Logger log = Logger.getLogger("fgsms.Agents");

    public void execute(JobExecutionContext jec) throws JobExecutionException {

        AuxHelper.TryUpdateStatus(true, "urn:fgsms:ServiceMixJMXAgent:" + SLACommon.GetHostName(), "OK", true, PolicyType.STATUS, AuxHelper.UNSPECIFIED, SLACommon.GetHostName());

        try {
            KeyNameValueEnc item = DBSettingsLoader.GetPropertiesFromDB(true, "SMXJMXAgent", "URLs");
            String t = null;
            String[] urls = null;
            try {
                if (item != null) {
            ...
            if (urls != null) {
                for (int i = 0; i < urls.length; i++) {
                    ...
                    org.miloss.fgsms.agents.smxviajmxagent.FgsmsSMXviaJMXAgent.Fire(true, connecturl, modifiedurl);

                }
            }
        } catch (Exception ex) {
            log.log(org.apache.log4j.Level.WARN, "Could not start the SMX JMX Agent ", ex);
        }

        try {
            KeyNameValueEnc interval = DBSettingsLoader.GetPropertiesFromDB(true, "SMXJMXAgent", "Interval");
...
/*
            ...
            if (sc.deleteJob(SMXJMXStarter.JOB_NAME, AuxConstants.QUARTZ_GROUP_NAME)) {
                JobDetail job = new JobDetail(SMXJMXStarter.JOB_NAME, AuxConstants.QUARTZ_GROUP_NAME, this.getClass());
                Trigger triger = TriggerUtils.makeSecondlyTrigger((int) (intinterval / 1000));
                triger.setStartTime(new Date());
                triger.setName(SMXJMXStarter.TRIGGER_NAME);
                ...
                log.log(org.apache.log4j.Level.WARN, "Error adjusting schedule for SMX JMX Agent ", ex);
        }
    }
}
