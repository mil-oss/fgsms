...
import org.miloss.fgsms.agents.CXFInterceotorOutClient;
import org.miloss.fgsms.agents.CXFInterceptorInClient;
import org.miloss.fgsms.agents.CXFInterceptorInService;
import org.miloss.fgsms.agents.CXFInterceptorOutService;
import org.tempuri.IService1;
import org.tempuri.OneWayMethodMethod;
import org.tempuri.Service1;

/**
 * self hosted service, one way soap method
 *
 * @author AO
 */
public class TC00163_JAXWS_CXFTest {
    ...
    static String firsthop_url = "http://" + Utility.getHostName() + ":9163/Service1";
    static String jettyurl = "http://localhost:9163/Service1";

    @org.junit.Test
    public void TC00163_1_MonitoredJaxWSClientToMonitorwsJaxWSServiceSelfHostedOneWay() throws Exception {
        TC00163_dowork(false, false);
    }

    @Ignore
    @org.junit.Test
    public void TC00163_2_MonitoredJaxWSClientToMonitorwsJaxWSServiceSelfHostedOneWay() throws Exception {
        TC00163_dowork(true, false);
    }

    @Ignore //this test doesn't make sense, a 1 way transaction has no response, therefore a resposne payload 
    //could never be recorded
    @org.junit.Test
    public void TC00163_3_MonitoredJaxWSClientToMonitorwsJaxWSServiceSelfHostedOneWay() throws Exception {
        TC00163_dowork(false, true);
    }

    @Ignore //this test doesn't make sense, a 1 way transaction has no response, therefore a resposne payload 
    //could never be recorded
    @org.junit.Test
    public void TC00163_4_MonitoredJaxWSClientToMonitorwsJaxWSServiceSelfHostedOneWay() throws Exception {
        TC00163_dowork(true, true);
    }

    void TC00163_dowork(boolean request, boolean response) throws Exception {
        ...
        org.apache.cxf.jaxws.EndpointImpl jaxwsEndpointImpl = (org.apache.cxf.jaxws.EndpointImpl) ep;
        org.apache.cxf.endpoint.Server server = jaxwsEndpointImpl.getServer();
        org.apache.cxf.endpoint.Endpoint cxfServiceEndpoint = server.getEndpoint();
        cxfServiceEndpoint.getOutInterceptors().add(new CXFInterceptorOutService());
        cxfServiceEndpoint.getInInterceptors().add(new CXFInterceptorInService());
        ...
        cxfEndpoint.getOutInterceptors().add(new CXFInterceotorOutClient());
        cxfEndpoint.getInInterceptors().add(new CXFInterceptorInClient());

        port.oneWayMethod(new OneWayMethodMethod(5));
        Thread.sleep(10000);
    ...
    private String VerifyMessagePayloads(String firsthop_url, int expectedrecords, boolean request, boolean response) throws Exception {
        ...
        if (recentMessageLogs.getLogs().getTransactionLog().size() != expectedrecords) {
            ...
            dump(recentMessageLogs.getLogs());
        }
        ...
        if (expectedrecords == 2 && recentMessageLogs.getLogs().getTransactionLog().size() == expectedrecords) {
            ...
            /*  //does not apply to one way to transactions
    ...
    private void dump(ArrayOfTransactionLog logs) {
        for (int i = 0; i < logs.getTransactionLog().size(); i++) {
            System.err.println(logs.getTransactionLog().get(i).getTransactionId() + " " + 
                    logs.getTransactionLog().get(i).getAction() + " " +
                    logs.getTransactionLog().get(i).getURL()+ " " +
                    logs.getTransactionLog().get(i).getTimestamp().getTime().toString()  );
        }
    }
}
