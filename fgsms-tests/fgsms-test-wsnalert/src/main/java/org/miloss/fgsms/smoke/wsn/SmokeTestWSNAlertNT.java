...
/**
 * non transactional service tests
 *
 * @author AO
 */
public class SmokeTestWSNAlertNT implements IWSNCallBack {

     String submgrurl = "http://localhost:8888/WSN-Broker/SubscriptionManager";

     public SmokeTestWSNAlertNT() throws Exception {
          callbackurl = "http://" + Utility.getHostName() + ":7777/WSNCallbackNT";
          p = new Properties();
          p.load(new FileInputStream(new File("../test.properties")));

     }
     private Properties p;
     static String callbackurl = "http://localhost:7777/WSNCallbackNT";
     static final String callbackurlListen = "http://0.0.0.0:7777/WSNCallbackNT";
     /**
      * @param args the command line arguments
      */
     private int count = 0;
     private int expectedcount = 1;
     //time out is set much higher on  this test because of aggregation delay
     int timeout = 450000;
     ...
     public static void main(String[] args) throws Exception {
          ...
          if (args.length > 0) {
               success = new SmokeTestWSNAlertNT().run(true);
          } else {
               success = new SmokeTestWSNAlertNT().run(false);
          }
          if (!success) {
               System.exit(1);
          }
     ...
     protected boolean run(boolean listenonly) throws Exception {
          ...
          if (!listenonly) {
               System.out.println("Sending some data to stimulate the NT SLA processor. I'll wait for a few minutes until I get a notification.");
               //trigger the alert
               SendData();
               ...
               while ((start + timeout) > System.currentTimeMillis()) {
                    ...
                    System.out.println((System.currentTimeMillis() - (start + timeout)) + " time remaining.");
                    Thread.sleep(15000);
               }
          } else {
     ...
     private void SetPolicy() throws ConfigurationException, ServiceUnavailableException, AccessDeniedException, DatatypeConfigurationException {
          ...
          rule.setClassName("org.miloss.fgsms.sla.rules.FaultsOverTimeGreaterThan");
          rule.getParameterNameValue().add(Utility.newNameValuePair("value", "1", false, false));
          rule.getParameterNameValue().add(Utility.newNameValuePair("duration", "300000", false, false));
